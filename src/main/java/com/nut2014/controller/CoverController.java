...
public class CoverController implements BaseController<Cover> {
    ...
    public PageBaseResponse<List<Cover>> getUserPage(int pageNum, int userId) {
        if (userId > 0) {
            PageHelper.startPage(pageNum, 20);
            ...
            return new PageBaseResponse<>(0, "success", null);
        }
    ...
    @RequestMapping("/likeCover")
    public BaseResponse<Cover> likeCover(@RequestParam("id") int id) {
        System.out.println(id + ">>>");
        Cover cover = dataService.get(id);
        cover.setLikeNumber(cover.getLikeNumber() + 1);
        int sqlCode = dataService.update(cover);
        return new BaseResponse<>(sqlCode, sqlCode > 0 ? "成功" : "失败", cover);
    }
